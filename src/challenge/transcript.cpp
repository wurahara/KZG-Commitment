...
BaseTranscript::BaseTranscript(const std::string_view &label) : Transcript(label) {}

void BaseTranscript::append_commitment(const std::string_view &label, const Commitment &commitment) {
...
void BaseTranscript::append_scalar(const std::string_view &label, const Scalar &scalar) {
    auto bytes = scalar.to_bytes();
    std::vector<uint8_t> scalar_vec{bytes.begin(), bytes.end()};
    this->append_message(label, scalar_vec);
}

Scalar BaseTranscript::challenge_scalar(const std::string_view &label) {
