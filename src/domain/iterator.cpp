...
ElementIterator::ElementIterator(EvaluationDomain domain,
                                 uint64_t init_power, const bls12_381::scalar::Scalar &init_value)
        : domain{std::move(domain)},
          power{init_power}, init_power{init_power},
          value{init_value}, init_value{init_value} {}

size_t ElementIterator::size() const {
    size_t size = 0;
    for (const bls12_381::scalar::Scalar &ele: *this) size++;
    return size;
}
...
bls12_381::scalar::Scalar ElementIterator::operator*() {
    return this->value;
}
